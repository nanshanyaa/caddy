<template>
  <section class="card panel drive-panel">
    <header class="drive-header">
      <div>
        <h2>文件管理</h2>
        <p class="muted">在这里上传文件、整理目录，并复制直链或分享链接。</p>
      </div>
    ...
    <section class="adapter-visibility card-lite">
      <div class="adapter-visibility-head">
        <h3>存储可用性</h3>
        <p class="muted">所有适配器都会显示出来，就算还没配置也能在这里看状态。</p>
      </div>
    ...
    >
      ...
      <p class="dropzone-title">拖拽文件到这里，或点击上传</p>
      <p class="muted">当前存储：{{ currentStorageLabel }} ｜ 目录：/{{ formatPathLabel(currentPath) }}</p>
    </section>
        ...
        <ul class="tree-list">
          ...
          >
            <button class="tree-link" @click="openPath(node.path)">
              <span>{{ formatUiText(node.name) }}</span>
      ...
      <article class="drive-main card-lite">
        <div class="drive-toolbar">
          <div class="breadcrumbs">
            ...
            >
              {{ formatUiText(crumb.name) }}
            </button>
          </div>
          <div class="toolbar">
            <input v-model.trim="search" placeholder="搜索当前目录" @keyup.enter="reloadExplorer" />
        ...
        <div class="folder-inline-list" v-if="folders.length">
          <article v-for="folder in folders" :key="folder.path" class="folder-card">
            <button class="folder-open" @dblclick="openPath(folder.path)" @click="openPath(folder.path)">
              <strong>{{ formatUiText(folder.name) }}</strong>
              <small>{{ formatFileCount(folder.fileCount) }}</small>
            </button>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="file in files" :key="file.name">
                ...
                <td>
                  <div class="file-col">
                    <strong>{{ formatUiText(file.metadata?.fileName || file.name) }}</strong>
                    <small>{{ file.name }}</small>
                  </div>
                </td>
                <td><span class="badge">{{ formatStorageType(file.metadata?.storageType) }}</span></td>
              ...
              <tr v-if="!loading && files.length === 0">
                <td colspan="6" class="empty">这个目录里还没有文件。</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const capabilityCards = computed(() => {
  const capabilityList = Array.isArray(status.value?.capabilities)
    ...
    : STORAGE_TYPES.map((item) => ({
      ...
      enableHint: '先去存储配置里完成接入。',
    }));

  return capabilityList.map((cap) => {
    const localMeta = STORAGE_TYPES.find((item) => item.value === cap.type);
    ...
    else if (configured) statusText = formatUiText(detail.message || '已配置，但当前不可用');
    return {
      ...cap,
      label: localMeta?.label || formatUiText(cap.label) || cap.type,
      description: localMeta?.description || '',
      configured,
      available,
      layerLabel,
      statusText,
      hint: available ? '现在就可以使用' : formatUiText(detail.message || cap.enableHint || '先完成配置再启用'),
    };
  });
});
...
function formatStorageType(value = '') {
  const text = String(value || '').trim();
  if (!text) return '未标记';
  return getStorageLabel(text.toLowerCase()) || formatUiText(text);
}
...
async function promptCreateFolder() {
  ...
  try {
    ...
    error.value = err.message || '新建文件夹失败';
  }
}
...
async function promptMoveFolder(folder) {
  const target = window.prompt('把文件夹移动到哪个路径（例如 assets/images）', folder.path);
  ...
  try {
    ...
    error.value = err.message || '移动文件夹失败';
  }
}
...
async function deleteFolderAction(folder) {
  ...
  try {
    ...
    if (String(err.message || '').includes('not empty')) {
      const recursive = window.confirm('这个文件夹里还有内容，要连同里面的文件一起递归删除吗？');
      if (!recursive) return;
      try {
...
async function promptMoveFile(file) {
  const target = window.prompt('把文件移动到哪个目录（留空表示根目录）', currentPath.value);
...
async function promptMoveSelected() {
  const target = window.prompt('把选中的文件移动到哪个目录（留空表示根目录）', currentPath.value);
  if (target == null) return;
  try {
...
async function deleteSelected() {
  ...
  if (!window.confirm(`确定删除选中的 ${selectedFileIds.value.length} 个文件吗？`)) return;
  try {
...
function formatFileCount(count = 0) {
  return `${count || 0} 个文件`;
}
