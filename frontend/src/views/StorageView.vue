<template>
  <section class="card panel storage-panel">
    <div class="panel-head storage-head">
      <div>
        <h2>存储配置</h2>
        <p class="muted">在这里维护上传后端、测试连接状态，并切换默认目标。</p>
        <p class="muted">如果需要聚合入口，WebDAV 更适合用来接 alist 或 openlist。</p>
      </div>
    ...
    <div class="storage-layout">
      <article class="storage-list card-lite">
        ...
        <p v-else class="muted">还没有任何存储配置。</p>
      </article>
      ...
      <article class="storage-editor card-lite">
        ...
        <form class="form-grid" @submit.prevent="submit">
          <label>
            名称
            <input v-model.trim="form.name" required placeholder="方便识别的名称" />
          </label>
          ...
          <div class="form-actions">
            <button class="btn" :disabled="saving">{{ saving ? '保存中...' : '保存设置' }}</button>
        ...
        <div v-if="draftTest" class="test-detail" :class="draftTest.connected ? 'ok' : 'fail'">
          <strong>{{ draftTest.connected ? '当前配置连接正常' : '当前配置连接失败' }}</strong>
...
async function testDraftConfig() {
  ...
  try {
    ...
    message.value = result?.connected ? '当前配置测试通过。' : '当前配置测试失败。';
  } catch (err) {
    draftTest.value = null;
...
async function testItem(id) {
  ...
  try {
    ...
    message.value = result?.connected ? '连接正常。' : '连接失败。';
  } catch (err) {
...
async function setDefault(id) {
  ...
  try {
    ...
    error.value = err.message || '设为默认失败';
  }
}

async function removeItem(id) {
  if (!window.confirm('确定删除这条存储配置吗？')) return;
...
function formatTestMessage(result) {
  const statusText = result.connected ? '连接正常' : '连接失败';
